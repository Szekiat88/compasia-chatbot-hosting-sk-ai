...
def main() -> None:
    input_sheet_name = "Sheet2"  # Specify the sheet name when the source is an Excel file
    ...
    for ticket_id in ticket_ids:
        try:
            ...
            # Persist any progress collected so far even if this ticket failed.
        ...
        if threads_data:
            last_thread = threads_data[-1]
            last_thread_id = str(last_thread.get("id"))
            try:
                detail_response = fetch_thread_detail(ticket_id, last_thread_id, headers)
            except requests.RequestException as exc:  # pragma: no cover - network dependent
                print(f"Skipping detail fetch for ticket {ticket_id}, thread {last_thread_id}: {exc}")
            else:
                detail_rows.append({"ticket_id": ticket_id, "thread_id": last_thread_id, **detail_response})
                content_rows.append(
                    {
                        "ticket_id": ticket_id,
        ...
        # Persist progress after each ticket is processed so intermediate results are captured.
        save_records(thread_rows, detail_rows, content_rows, output_path)
